                  ...
                  return (
                    <div className="flex flex-col gap-3 py-3">
                      ...
                      <div className="flex flex-col gap-2">
                        {metaCalls.map((tc, idx: number) => {
                          const toolName = tc.name || 'tool'
                          const toolArgs = tc.arguments || ''
                          const toolResult = tc.result || null

                          const parsedArgs = (() => {
                            try {
                              return JSON.parse(toolArgs) as Record<
                                string,
                                unknown
                              >
                            } catch {
                          ...
                          return (
                            <div
                              key={idx}
                              className="flex flex-col gap-2 px-4 py-3 bg-white/[0.03] border border-white/10 rounded-2xl text-[14px] shadow-sm hover:bg-white/[0.05] transition-all"
                            >
                                  ...
                                  <div className="flex flex-wrap gap-2 mt-1">
                                    {Object.entries(parsedArgs).map(
                                      ([key, value]) => (
                                        <div
                                          key={key}
                                          className="flex items-center gap-1.5 px-2.5 py-1 bg-white/5 border border-white/10 rounded-lg text-[12px]"
                                        >
