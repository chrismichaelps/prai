...
export function ProfileClient() {
  ...
  useEffect(() => {
    if (user && !chatsCountFetched.current) {
      ...
      fetch(
        `${process.env.NEXT_PUBLIC_SITE_URL || ''}/api/chat/chats/count?userId=${user.id}`,
      )
        .then((res) => res.json())
  ...
  const handleDeleteAllChats = async () => {
    ...
    if (
      !confirm(
        t('profile.confirm_delete_all_chats') ||
          'Are you sure you want to delete all your chats? This cannot be undone.',
      )
    )
      return
    ...
    try {
      ...
      setChatsCount(0)
      showToast(
        t('profile.all_chats_deleted') || 'All chats deleted',
        'success',
      )
    } catch (err) {
  ...
  const handleDeleteAccount = async () => {
    ...
    if (confirmHandle !== profile.handle) {
      showToast(
        t('profile.handle_mismatch') || 'Handle does not match',
        'error',
      )
      return
    }
    ...
    try {
      ...
      showToast(
        t('profile.account_deleted') || 'Account deleted successfully',
        'success',
      )
      signOut()
      ...
      showToast(
        t('profile.delete_account_error') || 'Failed to delete account',
        'error',
      )
    } finally {
      setIsDeleting(false)
  ...
  const handleArchiveAllChats = async () => {
    ...
    if (
      !confirm(
        t('profile.confirm_archive_all_chats') ||
          'Are you sure you want to archive all your chats?',
      )
    )
      return
    ...
    try {
      ...
      setChatsCount(0)
      showToast(
        t('profile.all_chats_archived') || 'All chats archived',
        'success',
      )
    } catch (err) {
      console.error('Error archiving chats:', err)
      showToast(
        t('profile.archive_error') || 'Failed to archive chats',
        'error',
      )
    } finally {
      setIsDeleting(false)
              ...
              {isEditing ? (
                  ...
                  <div className="space-y-5 mb-8">
                    <div>
                      <label
                        ...
                        className="block text-white/50 text-sm mb-2 font-medium"
                      >
                ...
                <div>
                  ...
                  <div className="flex flex-col items-center mb-8">
                    <motion.div className="w-28 h-28 rounded-full overflow-hidden border-4 border-white/20 shadow-xl bg-white/5 flex items-center justify-center">
                      {avatarUrl ? (
                        <>
