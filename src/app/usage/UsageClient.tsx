  ...
  return (
    <div className="space-y-4">
      <div className="flex justify-between items-end text-sm">
        <span className="text-white/40 font-bold uppercase tracking-[0.1em] text-[10px]">{label}</span>
    ...
    >
      <div className="relative z-10">
        <div className="flex items-center gap-3 mb-4">
          {color && <div className={cn("w-1.5 h-1.5 rounded-full shadow-[0_0_8px_currentColor]", color)} />}
          <span className="text-white/30 text-[11px] font-bold uppercase tracking-[0.1em]">{label}</span>
        </div>
    ...
    >
      ...
      <div>
        <p className="font-bold text-sm tracking-tight">
          {isAtCritical 
            ? t('usage.reached_limit')
            : t('usage.almost_limit')
          }
        </p>
...
function UsageContent() {
  ...
  const getBarColor = () => {
    ...
    if (usage.usage_percentage >= 70) return 'bg-yellow-400 shadow-yellow-400/20'
