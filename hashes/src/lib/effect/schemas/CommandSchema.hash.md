...
- **Default_Law:** `DEFAULT_CHAT_SETTINGS = { language: "es", webSearchEnabled: false }` — web search can be toggled on/off.
...
### [Change Notes — web search toggle]
- Added `webSearchEnabled` to `ChatSettingsSchema` with default `false`
- Updated `DEFAULT_CHAT_SETTINGS` to include `webSearchEnabled: false`
